...
class ActivityCreateJobTest < ActiveJob::TestCase
  ...
  test "Job ran with user with expired token should be refreshed and retried" do
    ...
    ActivityCreateJob.perform_now(3447675367, @user_with_valid_refresh.id, ENV["STRAVA_CLIENT_ID"], ENV["STRAVA_CLIENT_SECRET"])
  ...
  test "Job ran with user with invalid auth should not be retried" do
    ...
    ActivityCreateJob.perform_now(3447675367, @user_with_invalid_auth.id, ENV["STRAVA_CLIENT_ID"], ENV["STRAVA_CLIENT_SECRET"])
  ...
  test "Job ran with user with valid auth but throttled should schedule job" do
    ...
    assert_difference "Throttle.count", 1 do
      ActivityCreateJob.perform_now(3447675367, @user_with_valid_auth.id, ENV["STRAVA_CLIENT_ID"], ENV["STRAVA_CLIENT_SECRET"])
    end
    assert_enqueued_with(job: ActivityCreateJob, args: [3447675367, @user_with_valid_auth.id, ENV["STRAVA_CLIENT_ID"], ENV["STRAVA_CLIENT_SECRET"]], at: Time.now.utc.midnight.tomorrow)
  end
